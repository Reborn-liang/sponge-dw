...
public class SystemController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping(params = "goTypeGridByCode")
	public ModelAndView goTypeGridByCode(HttpServletRequest request) {
		...
		request.setAttribute("typegroup", group);
		return new ModelAndView("system/type/typeList");
	}
...
//    }
	...
	public AjaxJson delTypeGroup(TSTypegroup typegroup, HttpServletRequest request) {
        ...
        if (ListUtils.isNullOrEmpty(typegroup.getTSTypes())) {
        	...
        	cq.eq("TSTypegroup.typegroupcode", typegroup.getId());
        	cq.add();
        	List<TSType> types = systemService.getListByCriteriaQuery(cq, false);
        	for (TSType tsType : types) {
				systemService.delete(tsType);
			}
        	systemService.delete(typegroup);
            systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO);
...
//			depart.setOrgCode(orgCode);
        ...
        try {
	        ...
	        if("1".equals(upFlag)){
		        ...
		        MultipartFile mf = multipartRequest.getFile("file");// 获取上传文件对象
		    		...
		    		String newName = FileUtils.getRadomName(fileName);
		    		String savePath = file.getPath() + File.separator + newName;
					...
					String dbpath=bizPath+File.separator+nowday+File.separator+newName;
		    			...
		    			attachmentEntity.setTitle(FileUtils.getName(fileName));
		    			attachmentEntity.setExtend(FileUtils.getExtend(fileName));
		    			attachmentEntity.setContentType(contentType);
		    			attachmentEntity.setPath(dbpath);
		    			String id = (String) systemService.save(attachmentEntity);
		    			attachmentEntity.setId(id);
		    			attachmentEntity.setCreateDate(null);
					
					j.setObj(attachmentEntity);
		          ...
		          //如果是删除操作
		    		...
		    		if (!fileDelete.exists() || !fileDelete.isFile()) {
		    			...
		    			try {
	    					AttachmentEntity attachmentEntity = systemService.findUniqueByProperty(AttachmentEntity.class, "path", path);
	    					if(attachmentEntity != null) {
	    						systemService.delete(attachmentEntity);
	    					}
					} catch (Exception e) {}
		    		}else{
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="showOrDownloadByUrl",method = RequestMethod.GET)
	public void showOrDownloadByUrl(HttpServletResponse response,HttpServletRequest request) throws Exception{
		...
		if("1".equals(flag)){
			...
			String fileName=dbName.substring(dbName.lastIndexOf(File.separator)+1);
