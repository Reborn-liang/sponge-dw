...
public class LoginController extends BaseController{
	...
	public static ConcurrentHashMap<String, Integer> loginTriedIpMap = new ConcurrentHashMap<String, Integer>();
...
//		userService.pwdInit(user, newPwd);
	...
	public AjaxJson checkuser(TSUser user, HttpServletRequest req) {
		...
		String t = (String)session.getAttribute("NEED_VERIFY_CODE");
		if (!ErpConfig.isDebug() && t != null && "Y".equals(t) && StringUtils.isEmpty(randCode)) {
			j.setMsg(mutiLangService.getLang("common.enter.verifycode"));
			j.setSuccess(false);
		} else if (!ErpConfig.isDebug() && t != null && "Y".equals(t)
				&& !randCode.equalsIgnoreCase(String.valueOf(session.getAttribute("randCode")))) {
...
//			}
			...
			*/
			...
			addLoginTryingTimes(request);
			return "login/login";
		}

	}
	
	private void addLoginTryingTimes(HttpServletRequest request) {
		String ip = HttpUtils.getIp(request);
		Integer times = loginTriedIpMap.get(ip);
		if(times == null) {
			times=0;
		}
		if (times >= 3) {
				request.getSession().setAttribute("NEED_VERIFY_CODE", "Y");
			}else {
				request.getSession().setAttribute("NEED_VERIFY_CODE", "N");
			}
		loginTriedIpMap.put(ip, times.intValue()+1);
	}
	...
	public ModelAndView home(HttpServletRequest request) {
		...
		List<TSRoleUser> userRoles = systemService.findByProperty(TSRoleUser.class, "TSUser.id", currLoginUser.getId());
		
		for (TSRoleUser role : userRoles) {
			if (role.getTSRole().getRoleCode().equals(ErpConfig.getProperty("Defaul_Store_Mgr_Role"))
					|| role.getTSRole().getRoleCode().equals(ErpConfig.getProperty("Agent_Store_Mgr_Role"))
					|| role.getTSRole().getRoleCode().equals("HRMgr")
					|| role.getTSRole().getRoleCode().equals("HRAreaMgr")
					) {
				
				return new ModelAndView("main/storeHome");
			}else if (role.getTSRole().getRoleCode().equals("Supplier")){
					return new ModelAndView("main/supplierHome");
			}
		}
	...
	public void getPrimaryMenu(HttpServletResponse response) {
    	...
    	try {
    		...
    		log.info("getFunctionMap time="+(ets-sts)+"ms");
            String floor = "";
            if (primaryMenu == null) {
            	write.write(floor);
                return;
            ...
            for (TSFunction function : primaryMenu) {
                if(function.getFunctionLevel() == 0) {
                    ...
                    }else if("ERP".equals(lang_context)){
                    		floor += " <li><img class='imag1' src='plug-in/login/images/sale.png' /> "
                    			+ " <img class='imag2' src='plug-in/login/images/sale_up.png' style='display: none;' />" + " </li> ";
                    }else if("HR".equals(lang_context)){
                    		floor += " <li><img class='imag1' src='plug-in/login/images/hr.png' /> "
                    			+ " <img class='imag2' src='plug-in/login/images/hr_up.png' style='display: none;' />" + " </li> ";
                    }else if("采购库存".equals(lang_context)){
                    		floor += " <li><img class='imag1' src='plug-in/login/images/stocking.png' /> "
                    			+ " <img class='imag2' src='plug-in/login/images/stocking_up.png' style='display: none;' />" + " </li> ";
            ...
            log.info("===getPrimaryMenu time="+(ets-sts)+"ms");
		} catch (Exception e) {
