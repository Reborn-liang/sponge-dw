	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
		...
		if (excludeUrls.contains(requestPath)) {
			...
			for (String url : excludeUrls) {
				int starIndex = url.indexOf("*");
				if (starIndex >= 0) {
					String urlPart1 = url.substring(0, starIndex);
					String urlPart2 = url.substring(starIndex + 1, url.length());
					String apiReqPath = ResourceUtil.getRequestPathWithoutParams(request);
					if (apiReqPath.indexOf(urlPart1) == 0 && apiReqPath.indexOf(urlPart2) >= 0) {
						return true;
					}
				}
			}
