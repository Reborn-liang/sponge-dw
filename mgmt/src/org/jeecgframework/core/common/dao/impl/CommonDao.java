...
public class CommonDao extends GenericBaseCommonDao implements ICommonDao, IGenericBaseCommonDao {
    ...
    public void addFailedTimes(TSUser user) {
        ...
        TSUser user2 = (TSUser) session.get(TSUser.class, user.getId());
        if (user.getErrorNum() < 3) {
            user2.setErrorNum(user.getErrorNum() + 1);
        }
    ...
    public void resetFailedTimes(String id) {
        Session session = getSession();
        TSUser user2 = (TSUser) session.get(TSUser.class, id);
            user2.setErrorNum(0);
        session.update(user2);
    }
...
//				FileCopyUtils.copy(mf.getBytes(), savefile);
    ...
    public List<TreeGrid> treegridList(List all, TreeGridModel treeGridModel) {
        List<TreeGrid> treegrid = new ArrayList<TreeGrid>();
        for (Object obj : all) {
            ReflectHelper reflectHelper = new ReflectHelper(obj);
            TreeGrid tg = new TreeGrid();
