...
public class AutocompleteTag extends TagSupport {
	...
	private Boolean findTagByName = false;
	...
	public StringBuffer end() {
		...
		if(findTagByName) {
			nsb.append("$(\"input[name='"+name+"']\").autocomplete(\""+dataSource+"\",{");
		}else {
			nsb.append("$(\"#"+name+"\").autocomplete(\""+dataSource+"\",{");
		}
		nsb.append("max: 5,minChars: "+minLength+",width: 200,scrollHeight: 100,matchContains: true,autoFill: false,extraParams:{")
		...
		nsb.append("}); });");
		if(findTagByName) {
			nsb.append(" var p= $(\"input[name='"+name+"']\").val();");
			nsb.append(" console.log('p = ' + p);");
			nsb.append("function getTremValue(){return p;}");
		}else {
			nsb.append("function getTremValue(){return $(\"#"+name+"\").val();}");
		}
		nsb.append("</script>")
        .append("<input type=\"text\" name=\""+name+"\" id=\""+name+"\" datatype=\""+datatype+"\" nullmsg=\""+nullmsg+"\" errormsg=\""+errormsg+"\"/>");
	...
	public void setFindTagByName(Boolean findTagByName) {
