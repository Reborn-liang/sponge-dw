...
public class DataGridTag extends TagSupport {
	...
	public void setColumn(String title, String field, Integer width, String rowspan, 
		...
		if (null != operationCodes) {
			for (String MyoperationCode : operationCodes) {
				...
				if (operation != null) {
					...
					System.err.println("operation is null:" + MyoperationCode + ", want equal:" + field);
				}
			}
		}
		...
		if (!StringUtils.isBlank(dictionary)&&(!popup)) {
			if (dictionary.contains(",")) {
				...
				if (dic[0].indexOf("class:") == 0) {
					try {
						Class clazz = Class.forName(dic[0].split("class:")[1]);
						Method method = clazz.getDeclaredMethod(dic[1]);
						String id = "id";
						if (dic.length >= 3) {
							id = dic[2];
						}
						...
						Collection contents = (Collection) method.invoke(null);
						for (Object obj : contents) {
							try {
								text.append(SUReflectUtils.getFieldValue(obj, name) + ",");
								value.append(SUReflectUtils.getFieldValue(obj, id) + ",");
							} catch (Exception ex) {
								ex.printStackTrace();
							}
						}
						
						setColumn(field, text.toString(), value.toString());
					} catch (Exception e) {
						e.printStackTrace();
					}
				} else {
					String text = "";
					String value = "";
					String sql = "select " + dic[1] + " as field," + dic[2].replace("#", ",") + " as text from " + dic[0];
					if (dic.length == 4) {
						sql = sql + " " + dic[3].replace("#", ",");
					}
					// if(dic.length == 5){
					// String perStr = permissionByType(dic[4], dic[1], dic[3]);
	...
	private String permissionByType(String type,String field,String dictCondition){
...
//				}
	...
	public StringBuffer end() {
		...
		if (StringUtils.isNotEmpty(pageSizeList)) {
			sb.append("pageList:[" + pageSizeList + "],");
		} else {
			sb.append("pageList:[" + pageSize * 1 + "," + pageSize * 2 + "," + pageSize * 3 + "," + pageSize * 5 + ", 1000, 10000],");
		}
//		sb.append("pageList:[" + pageSize * 1 + "," + pageSize * 2 + "," + pageSize * 3 + "],");
		...
		if (columnList.size() > 0) {
			...
			sb.append("try{onSearchReseted();}catch(e){}");
			String func = name.trim() + "search();";
			sb.append(func);
			sb.append("}");
			
			sb.append("function " + name + "setCloumn(){");
			sb.append("var cols=$(\'#" + name + "\').datagrid('getColumnFields');");
			sb.append("var fields = new Array();");
			sb.append("var titles = new Array();");
			sb.append("var i = 0;");
			sb.append("for(var j = 0 ; j < cols.length; j++){");
			sb.append("var value = cols[j];");
			sb.append("if(value != 'id' && value != 'opt'){");
			sb.append("var row=$(\'#" + name + "\').datagrid('getColumnOption',value);");
			sb.append("fields[i] = value;");
			sb.append("titles[i] = row.title;");
			sb.append("i++;");
			sb.append("}");
			sb.append("}");
			sb.append("var fieldStr = '';");
			sb.append("$.each(fields, function(i, val){");
			sb.append("fieldStr+=val+',';");
			sb.append("}); ");
			sb.append("var titleStr = '';");
			sb.append("$.each(titles, function(i, val){");
			sb.append("titleStr+=val+',';");
			sb.append("}); ");
			sb.append("var height = parseInt(window.innerHeight - 20); ");
			sb.append("var saved = storage.get('"+name+"uncheckColumns');");
			sb.append("var saveStr = '';");
			sb.append("if(saved) {");
			sb.append("var reg = new RegExp('\\'','g');");
			sb.append("saveStr = saved.replace(reg,'');");
			sb.append("}");
			sb.append("var windowUrl = 'commonToolController.do?setShowCloumns&selectFields=' + encodeURIComponent(saveStr) + '&fields=' + encodeURIComponent(fieldStr) + '&titles=' + encodeURIComponent(titleStr); ");
			sb.append("createwindow('请选择您要显示的字段', windowUrl, 620, height); ");
			sb.append("}");
			
			sb.append("function didSelMultiCloumn(unCheckFileds,checkFileds){");
			sb.append("var reg = new RegExp('\\'','g');");
			sb.append("var saveStr = unCheckFileds.replace(reg,'');");
			sb.append("storage.set('"+name+"uncheckColumns', saveStr);");
			sb.append("var f =  unCheckFileds.split(',');");
			sb.append("$.each(f, function(index, field) {");
			sb.append("if(field != ''){");
			sb.append("var reg = new RegExp('\\'','g');");
			sb.append("var a = field.replace(reg,'');");
			sb.append("$(\'#" + name + "\').datagrid('hideColumn', a);");
			sb.append("}");
			sb.append("});");
			sb.append("var sf =  checkFileds.split(',');");
			sb.append("$.each(sf, function(index, field) {");
			sb.append("if(field != ''){");
			sb.append("var reg = new RegExp('\\'','g');");
			sb.append("var a = field.replace(reg,'');");
			sb.append("$(\'#" + name + "\').datagrid('showColumn', a);");
			sb.append("}");
			sb.append("});");
			sb.append("saveHeader();");
			sb.append("}");
			...
			if("group".equals(getQueryMode())){
				for (DataGridColumn col : columnList) {
					if (col.isQuery()) {
						...
						int width = col.getTitle().length() * 16;
						width =  width < 80 ? 80 : width;
						sb.append("<span style=\"vertical-align:middle;display:-moz-inline-box;display:inline-block;width: "+width+"px;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; \" title=\""+col.getTitle()+"\">"+col.getTitle()+"：</span>");
						if("single".equals(col.getQueryMode())){
							if(!StringUtil.isEmpty(col.getReplace())){
								...
								if(col.getDictionary().contains(",")&&col.isPopup()){
									...
									String sql = "select " + dic[1] + " as field," + dic[2].replace("#", ",")
											+ " as text from " + dic[0];
									if(dic.length == 4){
										sql = sql + " " +  dic[3].replace("#", ",");
									}
								...
								//	<input type="text" name="order_code"  style="width: 100px"  class="searchbox-inputtext" value="" onClick="inputClick(this,'account','user_msg');" />
									...
									if (dic[0].indexOf("class:") == 0) {
										try {
											...
											sb.append(generateSelect(col.getField(), contents, name, id));
										} catch (Exception e) {
											e.printStackTrace();
										}
									} else {
										...
										sb.append(generateSelect(col.getField(), list, "text", "field"));
									}
		...
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表单是组合查询
			...
			sb.append("<a href=\"#\" class=\"easyui-linkbutton\" iconCls=\"icon-edit\" onclick=\""+  name+ StringUtil.replaceAll("setCloumn()\">{0}</a>", "{0}", "配置显示列"));
	...
	private String generateSelect(String componentName, Collection contents, String textKey, String valueKey) {
		StringBuilder sb = new StringBuilder();
		sb.append("<select name=\"" + componentName.replaceAll("_", "\\.") + "\" WIDTH=\"100\" style=\"width: 104px\"> ");
		// 20180416 modify by dp for only one option error
//		if (contents.size() != 1) {
			sb.append(StringUtil.replaceAll("<option value =\"\" >{0}</option>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.please.select")));
//		}
		...
		for (Object obj: contents) {
			...
			sb.append(" <option value=\"" + map.get(valueKey) + "\">");
			sb.append(map.get(textKey));
			sb.append(" </option>");
		}
		sb.append("</select>");
		return sb.toString();
		...
		for (DataGridUrl dataGridUrl : list) {
			...
			if (StringUtil.isNotEmpty(exp)) {
				...
				for (String ShowbyField : ShowbyFields) {
					...
					else if ("ne".equals(exptype)) {
						sb.append("if($.inArray(rec['" + field + "'],[" + value + "])<0){");
					}
					else if ("empty".equals(exptype) && value.equals("'true'")) {
						sb.append("if(rec['" + field + "']==''){");
					}
					else if ("empty".equals(exptype) && value.equals("'false'")) {
						sb.append("if(rec['" + field + "']!=''){");
					}
					else if ("null".equals(exptype) && value.equals("'true'")) {
						sb.append("if(rec['" + field + "']==null){");
					}
					else if ("null".equals(exptype) && value.equals("'false'")) {
						sb.append("if(rec['" + field + "']!=null){");
					}
					
					
					else if ("ge".equals(exptype)) {
						sb.append("if(rec['" + field + "'] >= parseInt(" + value + ")){");
					}
					else if ("le".equals(exptype)) {
						sb.append("if(rec['" + field + "'] <= parseInt(" + value + ")){");
					}
					else if ("gt".equals(exptype)) {
						sb.append("if(rec['" + field + "'] > parseInt(" + value + ")){");
					}
					else if ("lt".equals(exptype)) {
						sb.append("if(rec['" + field + "'] < parseInt(" + value + ")){");
					}
					else if ("in".equals(exptype)) {
						sb.append("if($.inArray(rec['" + field + "'],[" + value + "])>=0){");
					}
				}
			}
			...
			if (OptTypeDirection.Del.equals(dataGridUrl.getType())) {
				if(dataGridUrl.getMessage()!=null) {
					sb.append("href+=\"[<a href=\'#\' onclick=delObj(\'" + url + "\',\'"+name+"\',\'"+ dataGridUrl.getMessage() +"\')>\";");
				}else {
			...
			if (null!=operationCodes) {
				for (String MyoperationCode : operationCodes) {
					...
					if (operation != null) {
						...
						System.err.println("operation is null:" + MyoperationCode);
					}
				}
			}
